...
public class ConnectedComponent<T> implements Graph<T> {
    ...
    /**
     * Gets all vertices in the component.
    ...
    public void updateVertex(T oldData, T newData) throws InvalidVertexException, ExistingVertexException {
        ...
        if (containsVertex(newData)) {
            throw new ExistingVertexException("New vertex data is already present in component.");
        }

        getVertex(oldData).setData(newData);
    }
    ...
    /**
     * @throws InvalidEdgeException If any of the vertices does not belong to the graph
     ...
     */
    @Override
    public boolean removeEdge(T vertexDataA, T vertexDataB) throws InvalidEdgeException, InvalidComponentException {
        if (!containsVertex(vertexDataA) || !containsVertex(vertexDataB)) {
     ...
     * @param <T> the type of data contained in all vertices
     * @throws InvalidEdgeException if any of the given edge vertices does not belong to the graph
     */
    static <T> Optional<Pair<ConnectedComponent<T>, ConnectedComponent<T>>> removeEdge(
            ConnectedComponent<T> component, T vertexDataA, T vertexDataB) throws InvalidEdgeException {
        if (!component.containsVertex(vertexDataA) || !component.containsVertex(vertexDataB)) {
            throw new InvalidEdgeException("At least one vertex do not belong to component.");
        }
