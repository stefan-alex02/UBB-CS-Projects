...
public class GameServicesImpl implements GameServices {
    ...
    public synchronized Game login(User user, GameObserver client) throws GameException {
        ...
        Collection<Word> words = wordRepository.chooseWords(5);

        List<Word> doubledWords = new ArrayList<>(words);
        doubledWords.addAll(words);
        Collections.shuffle(doubledWords);

        List<Configuration> configurations = new ArrayList<>();
        for (int i = 0; i < doubledWords.size(); i++) {
            configurations.add(new Configuration(
                    new Configuration.ConfigurationId(null, i + 1),
                    null,
                    doubledWords.get(i)));
        }

        return new Game(0, user, new ArrayList<>(), 0, 0, configurations);
    }
