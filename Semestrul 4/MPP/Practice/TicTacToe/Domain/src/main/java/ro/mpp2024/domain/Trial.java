...
@Table(name = "trials")
@Check(constraints = "row BETWEEN 1 AND 3 AND column BETWEEN 1 AND 3 AND symbol IN ('X', 'O')")
public class Trial implements Entity<Trial.TrialId> {
    ...
    public static class TrialId implements Serializable {
        ...
        public TrialId(Integer gameId, @Min(1) @Max(3) Integer row, @Min(1) @Max(3) Integer column) {
            this.gameId = gameId;
            this.row = row;
            this.column = column;
        }
    ...
    public Trial(TrialId id, Character symbol, Game game) {
        this.id = id;
        this.symbol = symbol;
        this.game = game;
    }
