...
const SAVE_MOVIES_STARTED = 'SAVE_MOVIES_STARTED';
const SAVE_MOVIES_SUCCEEDED = 'SAVE_MOVIES_SUCCEEDED';
const SAVE_MOVIES_FAILED = 'SAVE_MOVIES_FAILED';
...
const reducer: (state: MoviesState, action: ActionProps) => MoviesState =
  (state, { type, payload }) => {
    switch(type) {
      ...
      case SAVE_MOVIES_STARTED:
        return { ...state, savingError: null, saving: true };
      case SAVE_MOVIES_SUCCEEDED:
...
export const MovieProvider: React.FC<MovieProviderProps> = ({ children }) => {
  ...
  async function saveMovieCallback(movie: MovieProps) {
    try {
      ...
      dispatch({ type: SAVE_MOVIES_STARTED });
      const savedMovie = await (movie.id ? updateMovie(movie) : createMovie(movie));
      log('saveMovie succeeded');
      dispatch({ type: SAVE_MOVIES_SUCCEEDED, payload: { movie: savedMovie } });
    } catch (error) {
      log('saveMovie failed');
      dispatch({ type: SAVE_MOVIES_FAILED, payload: { error } });
    }
  }
  ...
  function wsEffect() {
    ...
    const closeWebSocket = newWebSocket(message => {
      ...
      if (event === 'created' || event === 'updated') {
        dispatch({ type: SAVE_MOVIES_SUCCEEDED, payload: { movie: movie } });
      }
    });
    return () => {
