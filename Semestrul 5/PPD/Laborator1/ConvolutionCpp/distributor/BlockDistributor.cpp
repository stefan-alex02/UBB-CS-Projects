#include "BlockDistributor.h"
#include "../thread/BlockConvolutionThread.h"
#include <thread>
#include <cmath>

template <typename TMatrix, typename TFilter>
void BlockDistributor<TMatrix, TFilter>::distribute(TMatrix F, int n, int m, TMatrix V, TFilter C, int k, int nrThreads, Technique technique) {
    ...
    for (int startRow = 0; startRow < n;) {
        ...
        for (int startCol = 0; startCol < m;) {
            ...
            threads[t++] = std::thread([&, startRow, endRow, startCol, endCol]() {
                blockConvolutionThread(F, V, n, m, C, k, startRow, endRow, startCol, endCol);
            });
            startCol = endCol;
        }
        startRow = endRow;
